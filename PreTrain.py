...
# WSL 下训练时取消下面一行注释
train_val_filepath_array, train_val_labels = path2target(r'/home/renjixiong/Model_Data/DataSet/pretrain_data/train')
test_data, test_label = path2target(r'/home/renjixiong/Model_Data/DataSet/pretrain_data/test')
...
# 将数据路径和对应标签打包成元组列表
train_val_data_pairs = list(zip(train_val_filepath_array, train_val_labels))
# 随机打乱元组列表
random.shuffle(train_val_data_pairs)
# 计算每个字符串列表应包含的元素数量
total_count = len(train_val_data_pairs)
list1_count = int(total_count * 0.8)
# list2_count = int(total_count * 0.2)
# 使用切片操作将元组列表分割成2个比例为8:2的元组列表
lists = [train_val_data_pairs[:list1_count], train_val_data_pairs[list1_count:]]
# 分别提取字符串列表和数字列表
